...
class OpenAI:
    ...
    # DALL-E 2
    def image_creation(self, prompt):
        images = openai.Image.create(prompt=prompt, **PARAMS["dall_e"])
        return images.get("data")

    # Whisper
    def audio_transcription(self, audio):
        audio = open(audio, "rb")
        transcript = openai.Audio.transcribe(file=audio, **PARAMS["whisper"])
        return transcript
